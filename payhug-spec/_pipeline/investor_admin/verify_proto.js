...
async function main(){
  ...
  /* ── 3) 엑셀 4건 — 원본처럼 중간 화면 없이 즉시 파일이 나오는지 실측 ──
     판정 = 실물 파일이 디스크에 떨어지고(바이트 일치) 화면이 미리보기로 넘어가지 않는다. */
  const EXPECT = {'assets-status':'투자자산현황_2026-08-27_2026-08-27.xlsx',
                  'assets-merchant':'가맹점별투자자산_2026-08-27_2026-08-27.xlsx',
                  'profit-status':'투자수익현황_2026-08-21_2026-08-27.xlsx',
                  'profit-daily':'일별투자수익_2026-08-21_2026-08-27.xlsx'};
  const FROM = {'assets-status':'invest-assets','assets-merchant':'invest-assets',
                'profit-status':'invest-profit','profit-daily':'invest-profit'};
  for(const key of Object.keys(EXPECT)){
    fs.readdirSync(DL).forEach(f => { try{ fs.unlinkSync(path.join(DL, f)); }catch(e){} });
    await evalJS('go(' + JSON.stringify(FROM[key]) + ', "default"); return 1;');
    await sleep(250);
    const click = await evalJS('var b=document.querySelector(\'[data-act="xls-open"][data-xls="' + key + '"]\');' +
    ...
    for(let i = 0; i < 25 && !got; i++){
      await sleep(200);
      const f = path.join(DL, EXPECT[key]);
      if(fs.existsSync(f) && fs.statSync(f).size > 0) got = f;
    }
    await sleep(500);
    const src = path.join(REPO, 'assets/xlsx', EXPECT[key]);
    const direct = (view === FROM[key]);
    R.downloads.push({key, expect:EXPECT[key], click, viewAfter:view, noPreviewScreen:direct,
  ...
  console.log('== 다운로드 4 =='); R.downloads.forEach(d => console.log(' ', line(d.key + ' → ' + d.file + ' ' + d.bytes + 'B', d.pass)));
