# -*- coding: utf-8 -*-
"""플랫폼별 평균 금융일수(Duration)와 예시 데이터의 플랫폼 구성 — W금융일수의 유일한 출처.

W금융일수는 금융 일반 용어로 `Duration` 이다. 가중치는 투자액(= 선정산대금)이다.
...
근거 2 — 로스터 구성비의 원천은 금액 실측이다 (Figma node 2782:5879 · 하루 1가맹점치)
    ...
    이 구성으로 낸 w금융일수 = 3.039607397240…일. 범위 2.0 ~ 6.2 안에 든다.
...
근거 2-b — 참고값으로만 남기는 대표 엑셀 비중 (조현준 `정산주기.xlsx` `비중` D4:H6)
    ...
    로스터 구성비의 원천으로 쓰지 않는다. 이 구성으로 낸 w금융일수 2.750406854861073일
...
#   플랫폼 평균 금융일수가 소수인 것은 건별 정수 금융일수가 섞여 나온 평균이기 때문이다.
...
def duration(mix):
    """플랫폼 구성비 → w금융일수(Duration). mix = (카드, 배민, 쿠팡이츠, 요기요), 합 1."""
...
MEASURED_W = duration(BOOK_MIX)                # 금액 실측 구성의 w금융일수 — 데이터의 기준값
