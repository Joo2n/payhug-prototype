# -*- coding: utf-8 -*-
"""로스터 16건·현황·일별·월별 — 전부 채권 원장(daily_ledger.py)에서 파생.

이 파일에는 금액도 W금융일수도 S입금부족율도 적혀 있지 않다. 원장이 만든 채권을 묶기만 한다.

    가맹점별 투자금액 = 그 가맹점의 미회수 Σ Ai
    W금융일수        = 미회수 Σ (Ai x Di) / 미회수 Σ Ai   (모집단이 투자금액과 같다)
    S입금부족율      = Σ SLi / Σ SAi   (표본 = 선정산일 D-20 ~ D-11)
    ty수익율         = 할인율 x 365 / W금융일수
    비중            = 각 금액 / 투자실행액 (소수1자리 반올림 · 잔차는 최대 금액 행이 흡수)

일별·월별 투자수익은 순지급액 앵커다(D-31) — 채권매입수수료 = 순지급액 x 할인율,
...
# ── 가맹점 로스터 16건 (금액 내림차순 · 1페이지 10 + 2페이지 6) ──────
...
def ratios(amounts, base):
    """소수1자리 반올림 후 잔차를 최대 금액 행에 흡수 — 합이 정확히 100.0."""
    raw = [r1(D(a) / D(base) * 100) for a in amounts]
    resid = D('100.0') - sum(raw)
    if resid:
        k = max(range(len(amounts)), key=lambda i: amounts[i])
        raw[k] += resid
    return raw
...
#   W = 미회수 Σ(Ai x Di) / 미회수 Σ Ai   ·   S = 표본집합 Σ SLi / Σ SAi
# 두 값의 모집단이 서로 다르다 — W 는 미회수분, S 는 선정산일 D-20~D-11 표본이다.
W_W  = W_RAW
S_W  = S_RAW
# ty = 할인율 x 365 / W. 화면에 뜨는 W(소수1자리)에서 낸 값이라 보는 사람이 화면 두 칸으로
...
def agg(rs, ec_days):
    ...
    return dict(exec=ex, profit=pr, repay=rp, w=r1(wv), wraw=wv, ty=r2(tv), tyraw=tv,
...
if __name__ == '__main__':
    ...
    print('비중 합계 %s (잔차 흡수: %s)' % (sum(SHARES), ROSTER[0][0]))
    ...
    print('투자실행액 행 — W %s (raw %s) · S %s%% (raw %s) · Ty %s%% · 비중 %s%%'
          % (r1(W_W), W_W.quantize(D('0.000001')), r2(S_W), S_W.quantize(D('0.000001')),
             r2(TY_W), EXEC_SHARE))
