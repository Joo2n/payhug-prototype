...
def build(idle=None, tag=''):
    ...
    rows = [
        ...
        ('기간', '어제 마감', asof, '날짜', 'daily_ledger.py ASOF'),
        ('기간', '선정산일 축 시작', '=%s-%d' % (D_, WINDOW - 1), '날짜', '축 %d일' % WINDOW),
        ('기간', '일별 표 시작', '=%s-26' % D_, '날짜', '전량 커버 구간 시작'),
        ('기간', '일별 표 종료', '=%s' % D_, '날짜', '어제 마감'),
        ('기간', '조회기간 시작', '=%s-6' % D_, '날짜', 'ceo_definitions.md default 일주일'),
        ('기간', '조회기간 종료', '=%s' % D_, '날짜', '어제 마감'),
        ('기간', 'S표본 시작', '=%s+1-20' % D_, '날짜', 'ceo_definitions.md 표본집합 d-20 (d = 오늘 = 어제 마감 + 1)'),
        ('기간', 'S표본 종료', '=%s+1-11' % D_, '날짜', 'ceo_definitions.md 표본집합 d-11 (d = 오늘 = 어제 마감 + 1)'),
    ...
    R5, R6 = (PER['⑤ wPYMR 투자 자산 대비 연환산 수익률 (정의)'],
    ...
    LATE = {
        ...
        '⑤ wPYMR 투자 자산 대비 연환산 수익률 (정의)': ('=입력!C%d' % IR['f5'], '입력 C%d' % IR['f5']),
    ...
    TILT = ('화면 원장은 요일·주차 규모 계수(SIZE)와 배달 의존도 틸트(TILT)로 하루치가 흔들리고, '
            '미회수 앵커가 어제 마감일에 걸린다. 이 통합문서는 하루치가 평평한 40일 창 모델이다.')
    ...
    res_rows = [
        ...
        ('조회기간 ④(%)', '=' + P('④ 투자실행금액 대비 ty수익율 (정의)'),
         ...
         TILT + ' ' + ROUNDUP),
        ('조회기간 ⑤(%)', '=' + P('⑤ wPYMR 투자 자산 대비 연환산 수익률 (정의)'),
         '기간집계 B%d' % PER['⑤ wPYMR 투자 자산 대비 연환산 수익률 (정의)'], V('주간 ⑤(%)'), P2,
         TILT + ' ' + ROUNDUP),
    ...
    RES0 = r + 1
    #   `읽는 법` 첫 표가 이 자리를 그대로 물어 온다. 두 곳이 어긋나지 않게 여기가 원천이다.
    RES = {lab: (RES0 + i, fmt) for i, (lab, _v, _p, _r, fmt, _w)
           in enumerate(res_rows)}
    ...
    MAPS = [
        ...
        ('투자자산 대비 ty수익율 (이미지의 ⑤)',
         ...
         % (PER['⑤ wPYMR 투자 자산 대비 연환산 수익률 (정의)'], IR['f5'])),
        ...
        ('투자실행금액 대비 ty수익율 (이미지의 ⑥)',
         ...
         % (PER['⑥ 투자실행금액 대비 ty수익율 (정의)'],
            ...
            PER['⑤ wPYMR 투자 자산 대비 연환산 수익률 (정의)'], IR['n3'])),
    ]
    r = 1
    ...
    widths(ws, {'A': 6, 'B': 22, 'C': 62, 'D': 56, 'E': 18, 'F': 18, 'G': 16})
    ...
    for i, lab in enumerate(gap):
        ...
        put(ws, r, 1, i + 1, M0)
        put(ws, r, 2, lab, bold=True)
        c = put(ws, r, 3, '=화면대조!H%d' % rr)
        c.alignment = Alignment(wrap_text=True, vertical='top')
        put(ws, r, 4, '화면대조 %d행' % rr)
        put(ws, r, 5, '=화면대조!C%d' % rr, fmt, fill=CHK)
        put(ws, r, 6, '=화면대조!E%d' % rr, fmt)
        put(ws, r, 7, '=화면대조!F%d' % rr, fmt)
    T0L = T0 + len(gap)
    GH = T0L + 2
    guide = [
        ...
        ('기간집계', '조회기간 집계 2~%d행 · 모집단별 W·Ty·S %d~%d행'
         ...
         % (PER['PA 투자실행금'], PER['④ 투자실행금액 대비 ty수익율 (정의)'],
            PER['⑤ wPYMR 투자 자산 대비 연환산 수익률 (정의)'], RG, RW, AGG['S입금부족율(%)'])),
    ...
    sym = [
        ...
        ('YMR(d-1)', '집계', '일별 연환산 수익률 = MR(d-1) x 365 / wD(d-1)', '일별 I열'),
        ...
        ('PYMR', '집계', '투자실행금액 대비 연환산 수익률(④) = PMR x 365 / PwD',
         '기간집계 B%d' % PER['④ 투자실행금액 대비 ty수익율 (정의)']),
        ('wPYMR', '집계', '투자 자산 대비 연환산 수익률(⑤) = PM x 365 / (Σ(Ai x Di) + PEC) '
