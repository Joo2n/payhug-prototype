...
const PAGE = 'terms-edit.html';
const CHROME = '/Applications/Google Chrome.app/Contents/MacOS/Google Chrome';
const HPORT = 8216, DPORT = 9216;
...
async function main() {
  ...
  const gen2 = path.join(REPO, '.termsedit-gen2.html');
  ...
  try {
    ...
    chk('A2', '항목 카드 수', await evalJS('return document.querySelectorAll(".card").length'), 45);
    chk('A3', '목차 항목 수', await evalJS('return document.querySelectorAll("#toc a").length'), 45);
    chk('A4', '대조용 갈고리 노출', await evalJS('return typeof window.__renderDoc'), 'function');
    chk('A5', '원문 인용 블록 수', await evalJS('return document.querySelectorAll("blockquote.q").length'), 45);
    chk('A6', '고칠 수 있는 자리가 있음',
        await evalJS('return document.querySelectorAll("[contenteditable]").length > 45'), true);
    chk('A7', '캡처가 실제로 뜸',
    ...
    await send('Page.navigate', { url: `http://127.0.0.1:${HPORT}/.termsedit-gen2.html` });
    await sleep(2500);

    chk('C1', '2세대도 자기 원본을 꺼냄',
        await evalJS('return (document.getElementById("st").textContent||"").indexOf("원본을 못 읽었습니다") < 0'), true);
    chk('C2', '2세대 카드 수', await evalJS('return document.querySelectorAll(".card").length'), 46);
    chk('C3', '2세대에 고친 풀이가 살아 있음',
        await evalJS('return document.querySelector(".card .plain").textContent'), '2세대 시험 문구');
