...
POP_S_TEXT = '선정산일이 오늘 기준 20일 전 ~ 11일 전인 표본'
...
def sec2(L, s1, drv):
    ...
    # D = 오늘 = 어제 마감(dl.ASOF) + 1
    chk('2', 'SAMPLE 시작 = D-%d' % SAMPLE_FROM,
        lo == dl.ASOF + timedelta(days=1) - timedelta(days=SAMPLE_FROM),
        '%s · 어제 마감 %s' % (lo, dl.ASOF))
    chk('2', 'SAMPLE 끝 = D-%d' % SAMPLE_TO,
        hi == dl.ASOF + timedelta(days=1) - timedelta(days=SAMPLE_TO),
        '%s · 어제 마감 %s' % (hi, dl.ASOF))
...
def sec6(L, s1, s2, s3, drv):
    ...
    for path, t in s2['files']:
        bad = t.replace(POP_S_TEXT, '선정산일이 오늘 기준 20일 전 ~ 10일 전인 표본')
