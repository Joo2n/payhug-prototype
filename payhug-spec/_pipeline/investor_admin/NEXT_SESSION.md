...
| C | **승인 문구와 다르게 된 2곳** — 「투자실행액」 카드 툴팁: 승인안 `Σ A_i` + `i | 정산예정일이 d 보다 뒤인 채권 · 2,240건` 대신 `A_i | 순지급액_i × (1 − r)` · `r | 0.11%`(빌더 지시에서 바뀜, d 는 화면에 안 쓰는 문제). 「순현금」 카드 툴팁 `EC_d = 20,000,000원` 은 만들지 않음. 낱장 4벌에는 카드 툴팁 자체가 없음(통합본·시연본만) | `build_app.py:1745` · `sync_assets_static.py` | 사용자 결정 → 재생성·배포·Figma 01-h 교체 |
