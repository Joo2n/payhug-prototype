...
| 투자자 공유 정리 (시연본만, 2026-09-09) | 탭 제목 「PayHug 투자자 어드민」 · 투자 자산 카드 아래 비중·보관 줄 0 · 현황 표 비중·보관 열 0(열 5, 빈 행 colspan 5) · 「투자자산 현황」 엑셀 두 열 0(5658 B, 레지스터 size 5.5 KB). 가맹점별 표 「비중」 열은 그대로. 통합본은 그대로 | `scripts/sync_prototype.py` `investor_share()`·`patch_assets_status_xlsx()` · `gate_prototype.js` 「투자자 공유 정리」 7검사 · `prep_fig.py share` · 시연본 `7a369bd` |
...
| N | **통합본에도 투자자 공유 정리를 넣을지** — 시연본만 정리됨. 넣으면 `build_app.py`(카드 sub·현황 표 열·`<title>`)·`sync_assets_static.py`·`build_sim_static.py`(시뮬 결과 카드·표)·`build_xlsx.py` `build_assets_status()`·전체본 첫 화면을 고치고, 변환기 `investor_share()`·`prep_fig.py share` 는 「이미 없음」 경고로 바꿔야 한다(지금은 앵커 없으면 fail) | 위 파일들 | 사용자 결정 (9/9 「추후 통합본 맞출 것인지만」) |
