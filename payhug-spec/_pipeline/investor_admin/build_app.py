...
# ── 수익 산정 기준 카드 노출 스위치 ───────────────────────────────
# False 면 투자 자산·투자 수익 두 화면에 카드를 싣지 않는다. 정적 낱장은
# sync_assets_static.py · sync_profit_static.py 의 drop_formula() 가 같은 카드를 걷어낸다.
# True 로 되돌리려면 원본 낱장 invest-assets.html 에 `<!-- 수익 산정 기준 -->` 카드가 있어야 한다.
SHOW_FORMULA = False
FORMULA = cut(_ia, '<!-- 수익 산정 기준 -->\n', '\n\n  </main>').rstrip() if SHOW_FORMULA else ''
...
<div class="modal-backdrop" data-modal="invest-assets-cert-confirm" hidden>
  <div class="modal md">
    ...
    <div class="modal-body">
      <p class="cert-desc">@@ASOF@@ 마감 기준 가맹점별 투자자산 내역으로 전자문서 발급.</p>
      <dl class="cert-info">
        <dt>문서명</dt><dd>투자자산 증명서</dd>
        <dt>기준</dt><dd class="mono">@@ASOF@@ 마감</dd>
...
/* 프리셋·달력 상한의 기준 — 일별 표 마지막 행 = 어제(마감 스냅샷 날). 어제 정산예정 채권은
   어젯밤 배치로 회수돼 마지막 행에 있다. 오늘은 화면에 표시하지 않는다. */
...
/* 일별 투자수익 원장 — 2026-03-01 ~ 08-27 180일. 생성기 daily_ledger.py.
   ...
   Ty수익율 = (투자 수익 / 투자실행금 x 100) x 365 / 가중평균 금융일수.
...
/* PwD 의 분자 Σ(A<sub>i</sub> x D<sub>i</sub>) — 날짜별 가중평균 금융일수를 6자리로 끊기 전 값이다.
...
/* 가중평균 금융일수와 입금부족률은 한 행에 나란히 서지만 모집단이 다르다.
...
var POP_W = {head:'D = Σ( A<sub>i</sub> × D<sub>i</sub> ) ÷ Σ A<sub>i</sub>', of:'보유 채권 전체', n:'@@POPW@@'};
var POP_S = {head:'LR = Σ L<sub>i</sub> ÷ Σ A<sub>i</sub>', of:'선정산일이 오늘 기준 20일 전 ~ 11일 전', n:'@@POPS@@'};
...
RENDER['certificate'] = function(){
  var exec = iaExecTotal(), cRatio = ratios(MERCHANTS, exec), i, h =
    '<thead><tr><th>가맹점</th><th>투자실행액 (원)</th><th>가중평균 금융일수</th><th>입금부족률</th><th>예상 연환산 수익률</th><th>비중</th></tr></thead><tbody>';
  for(i = 0; i < MERCHANTS.length; i++){
    var m = MERCHANTS[i];
...
function rollupBy(days, keyOf, labelOf){
  ...
  for(i = 0; i < out.length; i++){
    g = out[i];
    /* 가중평균 금융일수는 투자실행금 가중평균이다(단순 합이 아니다).
       Ty수익율은 대표 정의서 그대로 그 가중평균에서 되짚는다 — ty = SMR x 365 / SD,
       SMR = 버킷 투자수익 / 버킷 투자실행금, SD = 버킷 가중평균 금융일수.
...
/* 현황 카드 ④ 와 표 합계 행의 Ty수익율 = PMR x 365 / PwD (대표 정의서).
   PMR = 기간 투자수익 / 기간 투자실행금, PwD = 투자실행금 가중평균 금융일수.
...
function simSeedRows(){
  return [
    {plat:'card', amt:30400000, sd:'2026-08-26', dd:'2026-08-28'},
    {plat:'bm',   amt:35200000, sd:'2026-08-26', dd:'2026-08-29'},
    {plat:'cpe',  amt:9600000,  sd:'2026-08-26', dd:'2026-08-31'},
    {plat:'yo',   amt:4800000,  sd:'2026-08-26', dd:'2026-09-01'},
    {plat:'card', amt:15200000, sd:'2026-08-22', dd:'2026-08-24'},
    {plat:'bm',   amt:17600000, sd:'2026-08-22', dd:'2026-08-25'},
    {plat:'cpe',  amt:4800000,  sd:'2026-08-21', dd:'2026-08-26'},
...
function sheetData(key){
  ...
  if(key === 'assets-status'){
    ...
    rows.push({n:1, c:[{v:'가맹점별 투자자산 — ' + BASE_DATE + ' / ' + INVESTOR, c:'c-title', span:7}]});
    rows.push({n:2, c:[null, null, null, null, null, null, null]});
    rows.push({n:3, c:[{v:'가맹점', c:'c-head'}, {v:'투자실행액 (원)', c:'c-head r'}, {v:'가중평균 금융일수', c:'c-head r'},
                       {v:'입금부족률', c:'c-head r'}, {v:'예상 연환산 수익률', c:'c-head r'}, {v:'비중', c:'c-head r'}, null]});
    for(i = 0; i < MERCHANTS.length; i++){
      var m = MERCHANTS[i];
    ...
    rows.push({n:1, c:[{v:'투자수익 현황 — ' + BASE_DATE + ' / ' + INVESTOR, c:'c-title', span:2}, null, null, null, null]});
    rows.push({n:2, c:[null, null, null, null, null, null, null]});
    ...
    rows.push({n:7, c:[{v:'연환산 수익률 (투자실행금액 대비)'}, {v:fx(tyE, 2) + '%', c:'c-num'}, null, null, null, null, null]});
    rows.push({n:8, c:[{v:'연환산 수익률 (투자 자산 대비)'}, {v:fx(tyA, 2) + '%', c:'c-num'}, null, null, null, null, null]});
    rows.push({n:9, c:[null, null, null, null, null, null, null]});
    rows.push({n:10, c:[null, null, null, null, null, null, null]});
    ...
    rows.push({n:3, c:[{v:GRAN_COL[PF.gran], c:'c-head'}, {v:'상환액', c:'c-head r'}, {v:'투자실행금', c:'c-head r'},
                       {v:'투자 수익', c:'c-head r'}, {v:'가중평균 금융일수', c:'c-head r'}, {v:'연환산 수익률', c:'c-head r'}, null]});
...
# 날짜 — 원천은 daily_ledger.ASOF(어제 마감) 한 줄이다. 파일명·기본 조회 기간이 그 파생을 읽는다.
...
for _k, _v in (('@@ASOF@@',   daily_ledger.ASOF_S),
    ...
    DOC = DOC.replace(_k, _v)
assert '@@ASOF' not in DOC and '@@WKFROM' not in DOC and '@@LASTDUE' not in DOC, '날짜 토큰 잔여'
