...
### 2026-06-25
- **[추가]** `ui_kits/cashnote/` — 캐시노트 통합 UI 배포
  - Claude Design 프로젝트 `2ffe205d`에서 임포트
  - AppMain.jsx + appData.js를 index.html에 인라인 통합 (MIME 이슈 우회)
  - 매출관리 장부, 총 비용/수익, 수수료 상세 화면 추가
- **[추가]** `assets/` 이미지 3종 — bank-hana.png, avatar-default.png, store-thumb.png
  - 계좌 입금 내역 화면 하나은행 로고 복구

### 2026-06-24
- **[초기 설정]** `ui_kits/payhug_app/` — 페이허그 앱 첫 배포
  - PayhugApp.jsx + appData.js 인라인 통합
  - 디자인 시스템 번들 (`_ds_bundle.js`) 임포트
  - `.nojekyll` 추가 (Jekyll 제외 버그 수정)
  - 루트 `index.html` 리다이렉트 설정
